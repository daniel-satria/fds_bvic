...
def flag_qr_10min(
    ...
    try:
        ...
        logger.error("Flag pembayaran QR 5 mins Process terminated.")
        return
    ...
    try:
        ...
        if not is_empty(flag_only):
            ...
            logger.info(
                "There is no daily flagged pembayaran QR 5 mins transaction found.")
            logger.info("Process finished")
    except Exception as e:
        logger.error(
            f"Error calculation flagged pembayaran QR 5 mins transactions: {e}.")
        logger.error("Flag pembayaran QR 5 mins Process terminated.")
        return
