...
def filter_date(
    df: pl.DataFrame,
    n_days: int = CONSTS.params_non_qr_10min.n_days,
...
def flag_non_qr_10min(
    ...
    rolling_window: int = CONSTS.params_non_qr_10min.rolling_window,
    threshold: int = CONSTS.params_non_qr_10min.threshold,
) -> None:
    """
    ...
    try:
        ...
        burst_end = (burst_end.with_columns(
            (pl.col(f"{transaction_date_col}") - pl.duration(minutes=5)).alias("t_start"))
        )
